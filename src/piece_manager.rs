...
impl PieceManager {
    ...
    async fn on_piece_finished_msg(&mut self, piece_msg: PieceIoMsg) {
        let index = self
            .pending_pieces
            .iter()
            .position(|&pid| pid == piece_msg.piece_id);
        ...
        self.finished_pieces.insert(piece_msg.piece_id);
    ...
    async fn on_register_msg(&mut self) {
